...
const AdicionarDocumento = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return documentos.filter((doc) => !term || doc.name.toLowerCase().includes(term))
  }, [search])
    ...
    <main style={styles.shell}>
      <section style={styles.sheet}>
        ...
        {tab === 'biblioteca' ? (
          <>
            ...
            <div style={styles.list}>
              {filtered.map((doc) => {
                ...
                return (
                  <button key={doc.id} type="button" style={styles.row(checked)} onClick={() => toggle(doc.id)}>
                    <span style={styles.check(checked)}>{checked ? '✓' : ''}</span>
                    <span style={styles.rowText}>{doc.name}</span>
                  </button>
                )
              })}
            </div>
          ...
          <div style={styles.footer}>
            <Button onClick={() => navigate('/colaborador/documentos/novo')}>Cadastrar novo documento</Button>
          </div>
        )}
      </section>
    </main>
